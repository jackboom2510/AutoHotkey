global Language1_HKL := "0x0000409"
global Language2_HKL := "0x000042A"
global CurrentLanguageIndex := unset
global CurrentHKL := DllCall("GetKeyboardLayout", "UInt", DllCall("GetWindowThreadProcessId", "UInt", WinGetID("A"),
"UIntP", 0))
if (CurrentHKL = Language1_HKL)
    CurrentLanguageIndex := 1
else if (CurrentHKL = Language2_HKL)
...
class KeyBindingUI {
    ...
    __New(Addition*) {
        ...
        OnMessage(0x0207, ObjBindMethod(this, "On_WM_MBUTTONDOWN"))
    }
    ...
    SetUpAll(Addition*) {
        ...
        SetupControls() {
            ...
            this.Checkbox[1] := KeyBindingUI._gui.AddCheckbox("xm", "Utilities")
            this.Checkbox[1].Value := 1
            this.Checkbox[2] := KeyBindingUI._gui.AddCheckbox("xm", "Disable PgUp && PgDown")
        ...
        SetupToolTips() {
            ...
            this.Checkbox[1].ToolTip := "Hotkeys for Utilities (Timer, ChangeProjectMode, ChangeLangLayout)"
    ...
    On_WM_MBUTTONDOWN(wPARAM, lPARAM, msg, hwnd) {
        if (GetKeyState("Alt", "P")) {
            PostMessage(0xA1, 0x2, lPARAM, , hwnd)
            return 0
        }
        return 1
    }
...
ChangeLangLayout() {
    global CurrentLanguageIndex
    if (CurrentLanguageIndex = 2) {
        PostMessage(0x50, 0x0001, Language1_HKL)
        CurrentLanguageIndex := 1
    } else {
        PostMessage(0x50, 0x0001, Language2_HKL)
        CurrentLanguageIndex := 2
    }
    return
}
