...
class _UI {
    ...
    SetUpAll() {
        ...
        SetupToolTips
        return
        SetupControls(btnSpacing := 10, lineSpacing := 10) {
            this.ddl := this.gui.AddDDL('', [])
            SetupListView
            SetupListView(filePath := this.ddl.Text) {
                this.searchBar := this.gui.AddEdit('',)
                this.filterDdl := this.gui.AddDDL('', [])
                this.IML := IL_Create(this.IML_size)
                this.lv := this.gui.AddListView('Checked', ['', 'Name', 'Extension', 'Type', 'Size'])
                this.lv.SetImageList(this.IML)
            }
            this.gui.AddButton()
            this.gui.AddButton('',)
            this.gui.AddButton('',)
            ; this.gui.AddEdit('w60 +Right').ToolTip := 'Adjust ' this.name ''s transparency (' this.transparency.value.min 'â€“' this.transparency.value.max ').'
