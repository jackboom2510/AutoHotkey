...
;--------------------------------
    ...
    FindText(&OutputX := "", &OutputY := ""
        ...
        , ini := { sx: sx, sy: sy, sw: sw, sh: sh, zx: zx, zy: zy, mode: mode, bits: bits, ss: ss.Ptr, s1: s1.Ptr, s0: s0
            .Ptr, allpos: allpos.Ptr, allpos_max: allpos_max, err1: err1, err0: err0, zoomW: zoomW, zoomH: zoomH }
        loop 2 {
  ...
  , int new_w, int new_h )
{
  ...
  return ok;
            ...
            else  ; <FindMultiColor> or <FindColor> or <FindShape>
            {
                ...
                for k1, v1 in arr {
                    ...
                    loop n1
                        ...
                        , x := this.Floor(c[2]), x := (x <= 0 || x > 1 ? 0 : Floor(9 * 255 * 255 * (1 - x) * (1 - x)))
                        , NumPut("uint", this.ToRGB(c[1]) & 0xFFFFFF | (!shape && InStr(v1, "-") = 1 ? 0x1000000 : 0),
                        p += 4)
                        , NumPut("uint", (InStr(c[2], ".") ? x : this.Floor("0x" c[2]) | 0x1000000), p += 4)
                }
    ...
    PixelCount(x1 := 0, y1 := 0, x2 := 0, y2 := 0, ColorID := "", Variation := 0, ScreenShot := 1) {
        ...
        ini := { bits: bits, ss: ss.Ptr, s1: s1.Ptr, s0: s0.Ptr, allpos: 0, allpos_max: 0, err1: 0, err0: 0, zoomW: 1,
            zoomH: 1 }
        n := this.Floor(Variation), text := Format("##{:06X}$0/0/", n << 16 | n << 8 | n)
        . Trim(StrReplace(ColorID, "|", "/"), "- /")
    ...
    Gui(cmd, arg1 := "", args*) {
        ...
        switch cmd, 1 {
            ...
            case "ScreenShot":
                ...
                CoordMode "ToolTip"
                this.ToolTip(Lang["s9"], , 0, , { bgcolor: "Yellow", color: "Red", size: 48, bold: "bold", trans: 200,
                    timeout: 0.2 })
                return
                ...
                if (!MultiColor)
                    ...
                    loop arr.Length // 3
                        x1 := arr[i++], y1 := arr[i++], c1 := arr[i++], c := "0x" c1
                            , (A_Index = 1 && (x := x1, y := y1, rr := (c >> 16) & 0xFF, gg := (c >> 8) & 0xFF, bb := c &
                            0xFF))
                            , r := ((c >> 16) & 0xFF) - rr, g := ((c >> 8) & 0xFF) - gg, b := (c & 0xFF) - bb
                            , s .= "," (x1 - x) "/" (y1 - y) "/" (FindShape ? 3 * r * r + 4 * g * g + 2 * b * b <= n :
                                c1)
                    txt := SubStr(s, 2)
                }
            ...
            case "LButtonDown":
                ...
                if (Modify && bg != "" && show[k1])
                    ...
                    _Gui := FindText_Capture
                    _Gui["SelGray"].Value := (((k2 >> 16) & 0xFF) * 38 + ((k2 >> 8) & 0xFF) * 75 + (k2 & 0xFF) * 15) >>
                    7
