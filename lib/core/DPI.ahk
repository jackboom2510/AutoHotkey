...
class DPI {
    ...
    static Standard := 96, WM_DPICHANGED := 0x02E0, MaximumPerMonitorDpiAwarenessContext := VerCompare(A_OSVersion,
        ">=10.0.15063") ? -4 : -3, DefaultDpiAwarenessContext := this.MaximumPerMonitorDpiAwarenessContext
    ...
    static GetForWindow(WinTitle?, WinText?, ExcludeTitle?, ExcludeText?) {
        ...
        */
        ...
        return (hMonitor := DllCall("MonitorFromWindow", "ptr", WinExist(WinTitle?, WinText?, ExcludeTitle?,
            ExcludeText?), "int", 2, "ptr") ; MONITOR_DEFAULTTONEAREST
    ...
    static GetMonitorHandles() {
        ...
        local Monitors := []
        return DllCall("User32\EnumDisplayMonitors", "Ptr", 0, "Ptr", 0, "Ptr", EnumProc, "Ptr", ObjPtr(Monitors),
        "Int") ? Monitors : false
     ...
     * @returns {Integer} 
     */
    static MonitorFromWindow(WinTitle?, WinText?, flags := 2, ExcludeTitle?, ExcludeText?) => DllCall(
        "MonitorFromWindow", "int", WinExist(WinTitle?, WinText?, ExcludeText?, ExcludeText?), "int", flags, "ptr")
    ...
    static GetForMonitor(hMonitor, monitorDpiType := 0) {
        local dpiX, dpiY
        if !DllCall("Shcore\GetDpiForMonitor", "Ptr", hMonitor, "UInt", monitorDpiType, "UInt*", &dpiX := 0, "UInt*", &
            dpiY := 0, "UInt")
            return dpiX
    }
    ...
    static PixelSearch(&OutputVarX, &OutputVarY, X1, Y1, X2, Y2, ColorID, Variation?) {
        local out
        this.SetThreadAwarenessContext(this.MaximumPerMonitorDpiAwarenessContext)
        , (out := PixelSearch(&OutputVarX, &OutputVarY, X1, Y1, X2, Y2, ColorID, Variation?)) && this.ToStandardExceptCoordModeScreen(
            A_CoordModePixel, &OutputVarX, &OutputVarY)
        return out
    }
    ...
    static ImageSearch(&OutputVarX, &OutputVarY, X1, Y1, X2, Y2, ImageFile, dpi?, imgDpi?, &imgW?, &imgH?) {
        ...
        ImgPath := RegExMatch(ImageFile, "i)(?: |^)(?!\*(?:icon|trans|w|h|)[-\d]+)(.+)", &regOut := "") ? regOut[1] :
            ImageFile
        if !InStr(ImgPath, "\")
        ...
        if !IsSet(imgDpi)
            DllCall("gdiplus\GdipGetImageHorizontalResolution", "uint", pBitmap, "float*", &imgDpi := 0), imgDpi :=
            Round(imgDpi)
        ...
        if (out := ImageSearch(&OutputVarX, &OutputVarY, X1, Y1, X2, Y2, (dpi != imgDpi ? "*w" imgW " *h" imgH " " : ""
        ) ImageFile))
    ...
    static ControlGetPos(&OutX?, &OutY?, &OutWidth?, &OutHeight?, Control?, WinTitle?, WinText?, ExcludeTitle?,
        ExcludeText?) {
        local dpi
        this.SetThreadAwarenessContext(this.MaximumPerMonitorDpiAwarenessContext)
    ...
    static ControlClick(ControlOrPos?, WinTitle?, WinText?, WhichButton?, ClickCount?, Options?, ExcludeTitle?,
        ExcludeText?) {
        local x, y, regOut
        this.SetThreadAwarenessContext(this.MaximumPerMonitorDpiAwarenessContext)
        if IsSet(ControlOrPos) && ControlOrPos is String {
            if RegExMatch(ControlOrPos, "i)x\s*(\d+)\s+y\s*(\d+)", &regOut := "") {
                this.FromStandard(this.GetForWindow(WinTitle?, WinText?, ExcludeTitle?, ExcludeText?), &x := Integer(
                    regOut[1]), &y := Integer(regOut[2]))
                ControlOrPos := "X" x " Y" y
            }
        }
        ControlClick(ControlOrPos?, WinTitle?, WinText?, WhichButton?, ClickCount?, Options?, ExcludeTitle?,
            ExcludeText?)
    }
    ...
    static GuiOptScale(opt, targetDpi, fromDpi := this.Standard) {
        ...
        loop parse, opt, A_Space A_Tab {
            if RegExMatch(A_LoopField, "i)(w0|h0|h-1|xp|yp|xs|ys|xm|ym)$|(icon|hwnd)") ; these need to be bypassed
                ...
                out .= StrReplace(A_LoopField, number[2], this.ConvertCoord(&_ := Integer(number[2]), fromDpi,
                targetDpi)) A_Space
    ...
    static ToStandardExceptCoordModeScreen(CoordMode, &OutputVarX, &OutputVarY) => (CoordMode = "screen" || this.ToStandard(
        this.GetForWindow("A"), &OutputVarX, &OutputVarY))
    static FromStandardExceptCoordModeScreen(CoordMode, &OutputVarX, &OutputVarY) => (CoordMode = "screen" || this.FromStandard(
        this.GetForWindow("A"), &OutputVarX, &OutputVarY))
    static ConvertCoord(&coord, from, to) => ((IsNumber(coord) && coord := DllCall("MulDiv", "int", coord, "int", to,
        "int", from, "int")) || coord)

    ; Convert a point from standard to desired DPI, or vice-versa
    static FromStandard(dpi, &x, &y) => (IsInteger(x) && x := DllCall("MulDiv", "int", x, "int", dpi, "int", this.Standard,
        "int"), IsInteger(y) && y := DllCall("MulDiv", "int", y, "int", dpi, "int", this.Standard, "int"))
    static ToStandard(dpi, &x, &y) => (IsInteger(x) && x := DllCall("MulDiv", "int", x, "int", this.Standard, "int",
        dpi, "int"), IsInteger(y) && y := DllCall("MulDiv", "int", y, "int", this.Standard, "int", dpi, "int"))
     ...
     * @returns {Integer} 
     */
    static GetScriptAwareness() => DllCall("GetAwarenessFromDpiAwarenessContext", "ptr", DllCall(
        "GetThreadDpiAwarenessContext", "ptr"), "int")
    ...
    static PhysicalToLogicalPointForPerMonitorDPI(&x, &y, WinTitle?, WinText?, ExcludeTitle?, ExcludeText?) {
        ...
        DllCall("PhysicalToLogicalPointForPerMonitorDPI", "ptr", IsSet(WinTitle) && IsInteger(WinTitle) ? WinTitle :
            WinExist(WinTitle?, WinText?, ExcludeTitle?, ExcludeText?), "int64P", &pt64)
    ...
    static LogicalToPhysicalPointForPerMonitorDPI(&x, &y, WinTitle?, WinText?, ExcludeTitle?, ExcludeText?) {
        local pt := Buffer(8)
        NumPut("int", x, "int", y, pt)
        DllCall("LogicalToPhysicalPointForPerMonitorDPI", "ptr", IsSet(WinTitle) && IsInteger(WinTitle) ? WinTitle :
            WinExist(WinTitle?, WinText?, ExcludeTitle?, ExcludeText?), "ptr", pt)
        x := NumGet(pt, 0, "int"), y := NumGet(pt, 4, "int")
    }
    ...
    static AdjustWindowRectExForDpi(&x1, &y1, &x2, &y2, dpi) {
        ...
        x1 := NumGet(rect, 0, "int"), y1 := NumGet(rect, 4, "int"), x2 := NumGet(rect, 8, "int"), y2 := NumGet(rect, 12,
            "int")
    }
