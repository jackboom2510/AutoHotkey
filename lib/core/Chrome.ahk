...
class Chrome {
    ...
    __New(URLs := '', Flags := '', ChromePath := '', DebugPort := 9222, ProfilePath := '') {
        ...
        if instance := Chrome.FindInstance(exename, DebugPort) {
            ...
            for url in URLs
                http.Open('PUT', 'http://127.0.0.1:' this.DebugPort '/cJSON/new?' url), http.Send()
            return
        }
    ...
    GetPageList() {
        http := Chrome._http
        try {
            http.Open('GET', 'http://127.0.0.1:' this.DebugPort '/cJSON')
            http.Send()
            return cJSON.parse(http.responseText)
        } catch
            return []
    }
    ...
    NewPage(url := 'about:blank', fnCallback?) {
        http := Chrome._http
        http.Open('PUT', 'http://127.0.0.1:' this.DebugPort '/cJSON/new?' url), http.Send()
        if ((PageData := cJSON.parse(http.responseText)).Has('webSocketDebuggerUrl'))
    ...
    ClosePage(opts, MatchMode := 'exact') {
        ...
        switch Type(opts) {
            case 'String':
                return (http.Open('GET', 'http://127.0.0.1:' this.DebugPort '/cJSON/close/' opts), http.Send())
            case 'Map':
                if opts.Has('id')
                    return (http.Open('GET', 'http://127.0.0.1:' this.DebugPort '/cJSON/close/' opts['id']), http.Send())
            case 'Object':
                if opts.HasProp('id')
                    return (http.Open('GET', 'http://127.0.0.1:' this.DebugPort '/cJSON/close/' opts.id), http.Send())
        }
        for page in this.FindPages(opts, MatchMode)
            http.Open('GET', 'http://127.0.0.1:' this.DebugPort '/cJSON/close/' page['id']), http.Send()
    }

    ActivatePage(opts, MatchMode := 'exact') {
        http := Chrome._http
        for page in this.FindPages(opts, MatchMode)
            return (http.Open('GET', 'http://127.0.0.1:' this.DebugPort '/cJSON/activate/' page['id']), http.Send())
    }
    /**
    ...
    class Page extends WebSocket {
        ...
        Call(DomainAndMethod, Params?, WaitForResponse := true) {
            ...
            if !ID := this._index += 1
                ID := this._index += 1
            this.sendText(cJSON.stringify(Map('id', ID, 'params', Params ?? {}, 'method', DomainAndMethod), 0))
            if (!WaitForResponse)
                return
            ...
            if (response.Has('error'))
                throw Error('Chrome indicated error in response', , cJSON.stringify(response['error']))
        ...
        Evaluate(JS) {
            ...
            if (response is Map) {
                if (response.Has('exceptionDetails'))
                    throw Error(response['result']['description'], , cJSON.stringify(response['exceptionDetails']))
        ...
        Close() {
            ...
            http := Chrome._http, http.Open('GET', 'http://127.0.0.1:' m[1] '/cJSON/close/' m[2]), http.Send()
            this.__Delete()
        }

        Activate() {
            http := Chrome._http, RegExMatch(this.url, 'ws://[\d\.]+:(\d+)/devtools/page/(.+)$', &m)
            http.Open('GET', 'http://127.0.0.1:' m[1] '/cJSON/activate/' m[2]), http.Send()
        }
